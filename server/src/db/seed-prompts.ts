...
/**
 * Test Quality Reviewer (L02). Mirrors
 * `docs/agent-prompts/test-quality-reviewer.md` — edit both together.
 */
...
/**
 * API Contract Reviewer (L02 homework).
 *
 * Deliberately RULE-FREE: it carries the role, the evidence discipline and
 * the severity ladder, and not one rule about compatibility, response shapes,
 * semver or deprecation. Those live in the four linked skills, which is what
 * makes the controlled experiment mean anything — the same agent without them
 * has to miss the breaking change. A rule that leaks in here quietly turns the
 * control run into a second treatment run.
 */
