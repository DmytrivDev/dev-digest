...
async function writeFileAt(root: string, rel: string, contents: string): Promise<void> {
  ...
  // `dirname` handles both separators — a hand-rolled lastIndexOf('/') finds
  // nothing on Windows, where join() emits '\', so the mkdir was skipped and
  // writeFile failed with ENOENT for every nested fixture path.
